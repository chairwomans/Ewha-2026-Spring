...
| [14](#team-14) | def | 연구 | 멀티턴 코딩 에이전트에서 Prefix Caching의 한계를 극복하는 AST 인식 KV Cache 재사용 연구|
...
| 항목 | 내용 |
|------|------|
| 프로젝트명 | 멀티턴 코딩 에이전트에서 Prefix Caching의 한계를 극복하는 AST 인식 KV Cache 재사용 연구 |
| 서비스명(브랜드) | |
| 트랙 | 연구 |
| 팀명 | def |
| 팀구성 | 서혜원, 신은서, 이재린 |
...
| 고객 (누구를 위해) | - 로컬 LLM으로 코딩 에이전트를 운용하는 개인 개발자 / 연구자 - API 비용 대신 온디바이스 추론을 선택한 팀 (스타트업, 보안 민감 조직) - NVIDIA GPU 기반 자체 인프라 운용 조직 |
...
| 사용 기술 | - 에이전트 프레임워크 : OpenHands SDK - 추론 런타임 : vLLM - 모델 : Qwen3-Coder-30B-A3B-FP8 - 타겟 하드웨어 : NVIDIA RTX 5090 × 2장 (VRAM 31.84GB × 2, Linux) - AST 파싱 : Tree-sitter Python 바인딩 (증분 파싱) - 지문 산출 : BLAKE3 해시 (α-rename 정규화 + S-expression 직렬화) - KV Cache 제어 : vLLM block manager 커스터마이징 + RoPE Δ-rotation 모듈 - 벤치마크 : SWE-bench Lite (50 멀티턴 trace, OpenHands 에이전트로 수집) - 평가 지표 : TTFT(ms/턴), AST 서브트리 hit rate(%), Peak KV 메모리(MB)|
| 기대 효과 | - Prefill latency 감소: AST 서브트리 단위 KV Cache 재사용으로 반복 등장하는 코드 블록의 재계산 제거 - 동적 메타정보 문제 해결: tool_call_id·타임스탬프 등으로 인한 prefix 불일치를 우회하여 멀티턴 세션 전반에서 cache hit 유지 - 위치 독립적 재사용: RoPE re-positioning으로 위치가 달라진 서브트리도 KV를 올바르게 재사용 - Long context 환경 대응: 히스토리가 누적되어 컨텍스트 길이가 증가해도 TTFT 선형 증가 억제|
